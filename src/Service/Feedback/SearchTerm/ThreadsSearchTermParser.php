...
class ThreadsSearchTermParser implements SearchTermParserInterface
{
    public function supportsSearchTerm(SearchTermTransfer $searchTerm, array $context = []): bool
    {
        ...
        if ($searchTerm->getType() === SearchTermType::threads_username) {
            return true;
        }
    ...
    {
        $result = preg_match('/^(?:(?:http|https):\/\/)?(?:www\.)?threads\.net\/@?(' . $this->getUsernamePattern(true) . ')[?\/]?/im', $url, $matches);
    ...
    public function parseWithGuessType(SearchTermTransfer $searchTerm, array $context = []): void
    {
        if ($this->supportsUrl($searchTerm->getText(), $username)) {
            $searchTerm
                ...
                ->setType(SearchTermType::threads_username)
            ;
        } elseif ($this->supportsUsername($searchTerm->getText())) {
            $searchTerm
                ->addType(SearchTermType::threads_username)
            ;
        }
    }
